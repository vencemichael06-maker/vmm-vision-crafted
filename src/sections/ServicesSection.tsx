      ...
      <div className="vmm-section-pad relative">
        <div className="vmm-container grid gap-10 md:grid-cols-12 md:gap-6 lg:gap-10">
          <div className="md:col-span-4">
            ...
            <h2 id="services-title" className="vmm-heading mt-4 max-w-[10ch]">
          ...
          <ol className="grid border-l border-t border-vmm-ink/30 sm:grid-cols-2 md:col-span-8 lg:grid-cols-4">
            {services.map(({ index, title, body, icon: Icon }) => (
              <li
                key={title}
                data-service-reveal
                className={`group relative min-h-[15rem] border-b border-r border-vmm-ink/30 p-6 transition-colors duration-300 hover:bg-vmm-ink hover:text-white md:min-h-[16rem] ${index === "01" ? "lg:bg-vmm-red" : ""}`}
              >
                <div className="flex items-start justify-between">
                  <span className="font-display text-2xl">{index}</span>
                  <Icon className="h-7 w-7" strokeWidth={1.6} aria-hidden="true" />
                </div>
                <h3 className="mt-10 max-w-[10ch] font-display text-2xl uppercase leading-[0.96] xl:text-3xl">
                  {title}
                </h3>
                <p
                  className={`mt-4 max-w-xs text-sm leading-6 text-current opacity-70 ${index === "01" ? "lg:opacity-100" : ""}`}
                >
                  {body}
                </p>
                <ArrowDownRight
                  className="absolute bottom-5 right-5 h-5 w-5 transition-transform group-hover:translate-x-1 group-hover:translate-y-1"
                  aria-hidden="true"
                />
              </li>
            ))}
      ...
      >
        <div className="vmm-container grid gap-10 md:grid-cols-12 md:gap-6 lg:gap-10">
          <div className="md:col-span-4">
            ...
            <h3 id="process-title" className="vmm-heading mt-4 max-w-[11ch]">
              A CLEAR PROCESS<span className="text-vmm-red">.</span> MEASURABLE RESULTS
              <span className="text-vmm-red">.</span>
            </h3>
          </div>

          <ol className="border-t-2 border-vmm-ink md:col-span-8 md:grid md:grid-cols-5">
            {process.map(({ index, label, body, icon: Icon }) => (
              <li
                key={index}
                data-service-reveal
                className="grid grid-cols-[3rem_1fr] gap-4 border-b border-vmm-ink py-5 md:block md:min-h-[17rem] md:border-r md:px-4 md:py-6 last:md:border-r-0"
              >
                <span className="font-display text-xl text-vmm-ink">{index}</span>
                <div>
                  <Icon
                    className="mb-4 mt-1 h-7 w-7 text-vmm-red md:mt-6"
                    strokeWidth={1.8}
                    aria-hidden="true"
                  />
                  <h4 className="font-display text-xl uppercase leading-none">{label}</h4>
                  <p className="mt-3 text-sm leading-6 text-vmm-ink/65 md:text-xs md:leading-5">
                    {body}
                  </p>
                </div>
              </li>
